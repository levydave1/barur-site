  ...
  "tracks": [
    {
      ...
      "currentRate": number,           // הריבית השנתית הנוכחית של המסלול נכון להיום, באחוזים (למשל 4.35) — זו הריבית שתשמש לחישוב, ראה הנחיה מיוחדת למטה
      "originRate": number | null,     // הריבית השנתית שהייתה בעת מתן ההלוואה במקור, באחוזים, אם מוצגת בנפרד — למידע בלבד, לא לחישוב
...
- חשוב מאוד — currentRate מול originRate: במסלולים משתנים/צמודים, דוחות בנקים ישראליים (למשל מזרחי-טפחות) מציגים בסעיף "מנגנון קביעת הריבית" טבלה עם שתי עמודות ריבית זו לצד זו לכל שורה, בפורמט כמו: "שיעור הריבית לתקופה: 2.700000% / 3.780000%". המספר הראשון (2.70%) הוא הריבית שהייתה בעת מתן ההלוואה בעבר — שים אותו ב-originRate. המספר השני (3.78%) הוא הריבית הנוכחית, נכון למועד החישוב של הדוח — שים אותו ב-currentRate, וזה תמיד המספר שצריך ללכת לפיו. אותו עיקרון חל על "שיעור ריבית העוגן" ו"שיעור התוספת לעוגן" — תמיד השני מבין שני המספרים בכל שורה כזו הוא העדכני. במסלול בריבית קבועה (כמו "קבועה לא צמודה") יש בדרך כלל רק ריבית אחת בלבד — במקרה כזה שים אותו הערך גם ב-currentRate וגם ב-originRate.
- דוגמה מספרית (מהדוח שממנו נלקח הפורמט הזה): אם כתוב "שיעור הריבית לתקופה: 2.420000% / 3.780000%" — currentRate = 3.78, originRate = 2.42. אל תחזיר 2.42 בתור currentRate בשום מקרה כזה.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const t of parsed.tracks) {
      if (typeof t.balance !== 'number' || typeof t.currentRate !== 'number') {
